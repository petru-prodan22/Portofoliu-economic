function repairedPop = repairPortfolio(pop, cfg)
    ...
    for i = 1:popSize
        ...
        % Final: Normalizam si aplicam plafonarea matematic corect
        if sum(w) > 0
            w = w / sum(w);
            ...
            while any(w > cfg.maxWeight) && iter < maxIter
                ...
                % Redistribuim excesul proportional activelor valide
            ...
            % --- REPARATIE NOUA: Curatare Dust Weights (Ponderi sub pragul minim) ---
            dustIdx = find(w > 0 & w < cfg.minWeight);
            if ~isempty(dustIdx)
                dustCapital = sum(w(dustIdx));
                w(dustIdx) = 0; % Le anulam complet
                
                % Redistribuim capitalul recuperat catre activele valide
                validIdx = find(w >= cfg.minWeight & w <= cfg.maxWeight);
                if ~isempty(validIdx)
